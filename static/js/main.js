...
(async () => {
  ...
  async function write_logs() {
    try {
      ...
      if (!data.urls)
        throw new Error("Function had a server related issue and failed!");
      ...
      try {
        for (let name in logs)
          add_service(
            data.urls.filter((u) => u.name === name)[0].url,
            name,
            true,
            logs[name]
          );
        ...
        for (let name in logs)
          add_service(
            "https://pyxfluff.dev",
            `${name} &bullet; <span style="color: red;">Expiring soon</span>`,
            true,
            logs[name]
          );
      }
